...
```bash
# Start infra services (Postgres, Redis, Weaviate, Dagster)
docker compose up -d
...
- [ ] Safe environment variables management in sandboxes so they are not exposed to the agents
- [ ] Storage management in sandbox for better context engineering in long sessions
- [ ] Dockerize application (FE + API)
- [ ] Artifacts management generated by agents (reports, graphs, etc.)
- [ ] Add generative UI in the chat interface (new artifacts event type)
- [ ] Investigate other agent harnesses or simple frameworks (DSPy, OpenAI Agents SDK, etc.)