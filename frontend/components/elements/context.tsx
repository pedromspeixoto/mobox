...
export const Context = ({ className, usage, ...props }: ContextProps) => {
  ...
  // Hide the component if there's no usage data at all
  const hasAnyTokens = (usage?.totalTokens ?? 0) > 0 ||
                       (usage?.inputTokens ?? 0) > 0 ||
                       (usage?.outputTokens ?? 0) > 0;
  const hasAnyCost = (usage?.costUSD?.totalUSD ?? 0) > 0;

  if (!usage || (!hasAnyTokens && !hasAnyCost)) {
    return null;
  }
